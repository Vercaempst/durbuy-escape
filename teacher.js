...
document.getElementById("setCityButton")?.addEventListener("click", async () => {
  ...
  const feedback = document.getElementById("currentCityInfo");

  if (!cityKey) {
    if (feedback) feedback.innerText = "Kies eerst een stad.";
    return;
  }
  ...
  try {
    ...
    if (feedback) {
      const city = getCityRecord(cityKey);
      feedback.innerText = "Actieve stad ingesteld op: " + (city?.name || cityKey);
    }
  } catch (error) {
    console.error("Fout bij activeren van stad:", error);
    if (feedback) {
      feedback.innerText = "Activeren van de stad mislukt.";
    }
  }
});
